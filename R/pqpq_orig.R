...
#' @keywords PQPQ
#'
#' @seealso \code{\link{pqpq}}

pqpq_orig <- function(pepDataFixed, confList){
  ...
  for (protein in ProteinsOfInterest) {
    ...
    if(plyr::empty(AbundanceOnly)) {
      warning("ERROR: Peptide confidences too low for quantitation")
      next
    }

    ## Filter peptides with NAs
    if (all(!(complete.cases(AbundanceOnly))))
    {
      warning('NAs in all high confidence peptides - unable to proceed')
      next
    }
    AbundanceOnly <- AbundanceOnly[complete.cases(AbundanceOnly),]
    AbundanceOnly2 <- AbundanceOnly2[complete.cases(AbundanceOnly2),]
